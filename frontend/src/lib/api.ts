// Typed API client for the GhostDraft backend. Uses native fetch; falls back to local demo mode.
...
function buildDemoCompletion(proxy: ProxyResponse, prompt: string, model: string): CompleteResponse {
  ...
  if (promptLower.includes("e2b") || promptLower.includes("safety") || promptLower.includes("narrative")) {
    response_raw = [
      ...
      `Regulatory note: ${first("AMENDMENT", "<AMENDMENT_1>")} and ${first("INTERIM", "<INTERIM_1>")} remain protected by GhostDraft.`
    ].join("\n");
    ...
    response_raw = [
      ...
      `Suggested next action: ask for a rewrite, timeline summary, risk memo, or regulatory-style response and I will use the GhostDraft-safe proxy path.`
    ].join("\n");
  }
